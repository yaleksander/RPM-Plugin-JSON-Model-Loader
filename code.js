...
{
    if (!!RPM.Scene.Map.current && !RPM.Scene.Map.current.loading && mapID != RPM.Scene.Map.current.id)
    {
        mixerList = [];
...
function tweakMaterial(model, opts)
{
    if (!!model.material)
    {
        model.castShadow = RPM.Common.Utils.defaultValue(opts.castShadow, model.castShadow);
        model.receiveShadow = RPM.Common.Utils.defaultValue(opts.receiveShadow, model.receiveShadow);
        model.material.opacity = RPM.Common.Utils.defaultValue(opts.opacity, model.material.opacity);
        model.material.side = RPM.Common.Utils.defaultValue(opts.side, model.material.side);
    ...
    for (var i = 0; i < model.children.length; i++)
        tweakMaterial(model.children[i], opts);
}
            ...
            function (gltf)
            {
                ...
                RPM.Core.MapObject.search(id, (result) =>
                {
                    if (!!result)
                    {
                        tweakMaterial(model,
                        {
                            castShadow: result.object.mesh.castShadow,
                            receiveShadow: result.object.mesh.receiveShadow,
                            side: THREE.FrontSide // is this a good idea?
                        });
                        model.position.set(0, new THREE.Box3().setFromObject(model).getSize(new THREE.Vector3()).y * 0.5, 0);
...
RPM.Manager.Plugins.registerCommand(pluginName, "Set opacity",
function setOpacity(id, opacity)
{
    if (busy)
        setTimeout(setOpacity, 16, id, opacity);
    else
    {
        if (opacity < 0)
            opacity = 0;
        ...
        {
            if (!!result)
                tweakMaterial(result.object.mesh, {opacity: opacity});
        }, RPM.Core.ReactionInterpreter.currentObject);
    }
});

RPM.Manager.Plugins.registerCommand(pluginName, "Set face side",
function setSide(id, side)
{
    if (id == -1)
        id = RPM.Core.ReactionInterpreter.currentObject.id;
    if (busy)
        setTimeout(setSide, 16, id, side);
    else
    {
        const model = getModel(id);
        if (!!model)
            tweakMaterial(model, {side: side});
    }
});
