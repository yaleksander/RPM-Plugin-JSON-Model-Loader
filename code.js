...
function enableShadows(model, cast, receive)
{
	if (!model.isScene)
	{
	...
	for (var i = 0; i < model.children.length; i++)
		enableShadows(model.children[i], cast, receive);
}
		...
		(
			...
			function (model)
			{
				RPM.Core.MapObject.search(id, (result) =>
				{
					if (!!result)
					{
						enableShadows(model, result.object.mesh.castShadow, result.object.mesh.receiveShadow);
